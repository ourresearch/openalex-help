...
for (const a of articles) {
  ...
  const row = { id: a.id, section, title: a.title, action, tab: tab || '', slug: '', target: '', reason, review,
    oldPath: new URL(a.html_url).pathname };
...
// _redirects fragment. STATIC rules only (exact canonical path from html_url +
// the bare-id variant) plus ONE catch-all splat for stale title-slug variants.
// Cloudflare Pages constraints discovered the hard way (2026-07-28):
//   - inline comments after a rule are invalid, and the parser stops processing
//     the file at the first invalid line (everything after silently dead);
//   - only 100 dynamic (wildcard) redirects are honored — one splat per article
//     (136) blows the cap. Static rules get 2,000.
const red = ['# Zendesk KB → new IA (generated by scripts/migrate.mjs — do not hand-edit this block)'];
for (const r of dispositions) {
  const target = r.action === 'migrate' ? `/${r.tab}/${r.slug}/` : r.action === 'redirect' ? r.target : '/help/';
  if (r.action === 'retire') red.push(`# retired: ${r.reason}`);
  red.push(`${r.oldPath}  ${target}  301`);
  red.push(`/hc/en-us/articles/${r.id}  ${target}  301`);
}
red.push('# catch-all: stale title-slug variants + any other old Guide path');
red.push('/hc/*  /help/  301');
...
if (current.includes(BEGIN)) {
  // Escape the markers — BEGIN contains regex metacharacters (parens, dots);
  // unescaped they silently never match and re-runs no-op the injection.
  const esc = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  current = current.replace(new RegExp(`${esc(BEGIN)}[\\s\\S]*?${esc(END)}`), block);
} else {
