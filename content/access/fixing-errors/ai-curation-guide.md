...
3. **Your sub-unit has no ROR record.** OpenAlex only mints institutions from [ROR](https://ror.org/). If a school/department/institute lacks its own ROR record, its strings can't map to it — [request the record at ROR](https://ror.org/curate/) (free), and don't force-map its strings to a wrong sibling in the meantime.
