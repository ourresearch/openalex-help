...
The file is **cumulative** — each release carries the full history, so you don't need to collect it daily. Apply it as: *remove these IDs from your copy of works*. That makes mirror deletions a direct lookup instead of a full ID-set diff (the reconcile methods below still work and remain the belt-and-braces option). One subtlety: occasionally a deleted work **returns to the corpus** — the same paper, under the same ID, typically because its hosting repository dropped out of harvesting and later came back. When that happens the ID is removed from the log (a live work never appears in it), so the file can occasionally shrink between releases. Don't treat an ID's disappearance *from the log* as a new deletion; the log is always the current set of dead IDs. And to be clear about what this is **not**: a work ID is never reassigned — W-IDs are minted from a forward-only sequence, so a deleted ID either stays dead or comes back as the *same* work, never as a different one.
