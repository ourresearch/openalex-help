...
> [!claude]
> Counting and grouping is a plain-language ask in Claude: *"How many works on X per year since 2015?"* or *"Which journals carry the most papers on Y?"* Each answer includes the exact query it ran. Set up once via the [AI agent connector](/access/connector/). (Totals of citations across a result set are website-only for now.)
