...
tags: ["reference"]
---
A **raw affiliation string** (RAS) is the exact affiliation text an author printed on a work, before OpenAlex matches it to an [institution](/entities/institutions/) — free text like `"Impactstory, Sanford, NC, USA"` or `"Massachusetts Institute of Technology"`, often messy and inconsistent. It's the raw input to institution disambiguation: the thing OpenAlex parses to figure out *which organizations* an author was affiliated with. Raw affiliation strings are a [component](/entities/component/) entity — they don't get their own OpenAlex ID. They live inside an [authorship](/entities/authorships/), in its [`raw_affiliation_strings`](/entities/authorships/#raw_affiliation_strings) list and its [`affiliations`](/entities/authorships/#affiliations) mapping. There's also a bespoke `raw_affiliation_strings.search` filter on [works](/entities/works/) and a raw-affiliation-strings search surface for querying them directly.
...
A raw affiliation string is essentially a string plus its matched-institution mapping, both of which live on the [authorship](/entities/authorships/). Being a component entity, it carries none of the [common fields](/entities/common-fields/) and has no OpenAlex ID.
...
- **`institution_ids`** *(List)* — the OpenAlex [institution](/entities/institutions/) IDs that string resolved to (empty if nothing matched).
...
This is the authoritative record of *which printed string produced which institutions* — the flattened [`institutions`](/entities/authorships/#institutions) list on the authorship loses that per-string provenance.

## In the API

Raw affiliation strings aren't a top-level entity endpoint you list or fetch — they surface as fields on an [authorship](/entities/authorships/) (select [`authorships`](/entities/works/#authorships) on a work) and through two search surfaces:

- **Search the raw text** with the `raw_affiliation_strings.search` filter on [works](/entities/works/): `filter=raw_affiliation_strings.search:impactstory` returns works whose authors printed that affiliation text — matching on the *raw string*, before institution disambiguation. Useful for finding an organization's works when its name never resolved cleanly to a ROR-backed institution.
...
See [Filtering](/api/filtering/) for filter syntax and [Searching](/api/searching/) for how `.search` behaves. To filter on the resolved institution itself (rather than the raw text), use `authorships.institutions.id` on [works](/entities/works/).
