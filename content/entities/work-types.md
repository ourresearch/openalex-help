...
Each work type is a small object. Fields shared with other entities are documented once on [Common fields](/entities/common-fields/).

### `id`
*String.* The [OpenAlex ID](/entities/overview/#the-openalex-id-scheme) for this type, e.g. `https://openalex.org/types/article`. See [Common fields](/entities/common-fields/#id).

### `display_name`
*String.* The human-readable name of the type — identical to the ID's final segment (`article`, `dataset`). See [Common fields](/entities/common-fields/#display_name).

### `description`
*String.* A one-line definition of the type (the same text shown in the [Values](#values) table above).

### `works_count`
*Integer.* How many [works](/entities/works/) carry this type. See [Common fields](/entities/common-fields/#works_count).

### `cited_by_count`
*Integer.* Total citations across all works of this type. See [Common fields](/entities/common-fields/#cited_by_count).

### `works_api_url`
*String.* A ready-made [Works](/entities/works/) API URL listing every work of this type, e.g. `https://api.openalex.org/works?filter=type:article`.
