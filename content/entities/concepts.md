...
description: "What concepts were, why they're deprecated in favor of topics, and what every field on a concept object means."
tags: ["reference"]
source_id: "api-reference/concepts"
source_url: "https://developers.openalex.org/api-reference/concepts"
source_updated: "2026-02-17"
---
> **Deprecated.** Concepts are superseded by [Topics](/entities/topics/), OpenAlex's current subject-classification system. The concepts endpoints and the [`concepts`](/entities/works/#concepts) field on works still work, but concepts are frozen — no longer recomputed or expanded — so new works are not tagged with them and the concept tree is not maintained. Use [topics](/entities/topics/) (and the [aboutness](/entities/aboutness/) hierarchy of [domains](/entities/domains/), [fields](/entities/fields/), and [subfields](/entities/subfields/)) for anything new. This page is kept for the many links and workflows that still point at it.
...
This scheme was retired because it was hard to maintain and its accuracy was uneven, especially at the deeper levels. In 2024 OpenAlex replaced it with **[topics](/entities/topics/)** — a cleaner, curated four-level [aboutness](/entities/aboutness/) hierarchy ([domains](/entities/domains/) → [fields](/entities/fields/) → [subfields](/entities/subfields/) → [topics](/entities/topics/)) that classifies each work more reliably. Concepts are now frozen: the tree is no longer expanded, and derived fields such as [`counts_by_year`](#counts_by_year), [`summary_stats`](#summary_stats), [`ancestors`](#ancestors), and [`related_concepts`](#related_concepts) are no longer recomputed and are typically null on the live objects. See [Aboutness](/entities/aboutness/) for how the successor signals compare.

## Fields

This is the canonical dictionary of every field on a **concept** object. Fields shared with other entities are documented once on [Common fields](/entities/common-fields/); concept-specific notes are below. As noted above, several fields are no longer maintained and may come back empty.

### `id`
*String.* The [OpenAlex ID](/entities/overview/#the-openalex-id-scheme) for this concept, e.g. `https://openalex.org/C41008148`. See [Common fields](/entities/common-fields/#id).
...
### `display_name`
*String.* The concept's name, e.g. `Computer science`. See [Common fields](/entities/common-fields/#display_name).
...
### `international`
*Object.* The concept's `display_name` and `description` translated into many languages, keyed by [language code](/entities/languages/). No longer maintained — usually an empty object on live concepts.
...
### `works_count`
*Integer.* The number of works tagged with this concept. See [Common fields](/entities/common-fields/#works_count).

### `cited_by_count`
*Integer.* The total citations received by all works tagged with this concept. See [Common fields](/entities/common-fields/#cited_by_count).

### `summary_stats`
*Object.* Precomputed bibliometric indicators (`2yr_mean_citedness`, `h_index`, `i10_index`). See [Common fields](/entities/common-fields/#summary_stats). No longer recomputed — typically null on live concepts.

### `counts_by_year`
*List.* Works and citation counts for roughly the last ten years, one element per year. See [Common fields](/entities/common-fields/#counts_by_year). No longer recomputed — typically null on live concepts.

### `works_api_url`
*String.* A ready-made [Works API](/entities/works/#in-the-api) URL returning every work tagged with this concept, e.g. `https://api.openalex.org/works?filter=concepts.id:41008148`. A convenience link — the same query you'd build with the [`concepts.id`](/entities/works/#concepts) filter.

### `created_date`
*String.* The date this concept was added to OpenAlex (`YYYY-MM-DD`). See [Common fields](/entities/common-fields/#created_date).

### `updated_date`
*String.* The [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) UTC timestamp of the last change to this concept object. See [Common fields](/entities/common-fields/#updated_date).
...
To find the works carrying a concept, filter on the [Works](/entities/works/) endpoint:
