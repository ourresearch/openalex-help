...
* `Concept`: The Concept's [description](broken-reference), e.g. "the study of relation between plant species and genera"

You can change the author hint to the author's [last known institution](broken-reference) by adding the `author_hint` parameter with value set to `institution` ([read more](../search/search-authors.md#autocomplete-authors)).
...
All entity [filters](broken-reference) and [search](search-entities.md) queries can be added to autocomplete and work as expected, like:
