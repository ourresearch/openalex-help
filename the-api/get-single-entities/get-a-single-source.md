...
* Get the source with the [OpenAlex ID](./#the-openalex-id) `S137773608`:\
  [https://api.openalex.org/sources/S137773608](https://api.openalex.org/sources/S137773608)

That will return an [`Source`](broken-reference) object, describing everything OpenAlex knows about the source with that ID:
...
You can use `select` to limit the fields that are returned in a source object. More details are [here](../get-lists-of-entities/select-fields.md).
